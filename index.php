...
//funcion para agregar una nueva evaluacion
    ...
    if(!empty($token)){
        ...
        if($arrT['valido']){
            ...
            if(count($arrAlu)>= 1){
                ...
                /*foreach($GLOBALS['arr_campos_evaluacion_nn'] as $k=>$campo){
                    if(empty($arrJ[$campo])){
                        $arr_empty[$campo] = "No puede ser nulo";
                        $validos = 0;
                    }
...
$app->delete('/calificacion/{id}', function (Request $request, Response $response, array $args) {
    ...
    $token = $arrJ['token'];
    $arr = array();
    //$token = "94ad7a91c8fc26fef3e2311c7807e68959266488dadeffabf62cc1df4dad6d40";
